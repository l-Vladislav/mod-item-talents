...
-- (choice index); kills = item XP counter, flushed on save/logout (not per
-- kill) per DESIGN.md "Опыт предмета (kills)". No seed data — rows are
-- created/updated by the module at runtime, so this file only ensures the
-- schema exists (idempotent via CREATE TABLE IF NOT EXISTS).
